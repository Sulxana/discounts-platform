...
try
{
    ...
    builder.Host.UseSerilog((context, services, configuration) => configuration
        ...
        .WriteTo.Console(outputTemplate: "[{Timestamp:HH:mm:ss} {Level:u3}] [ReqId: {RequestId}] [CorrId: {CorrelationId}] {Message:lj}{NewLine}{Exception}", formatProvider: System.Globalization.CultureInfo.InvariantCulture));
    ...
    if (app.Environment.IsDevelopment())
    {
        ...
        app.UseSwaggerUI(options =>
        {
            ...
            {
                options.SwaggerEndpoint($"/swagger/{description.GroupName}/swagger.json", description.GroupName?.ToUpperInvariant() ?? string.Empty);
            }
        });
    }
    ...
    {
        Predicate = r => r.Name != null && r.Name.Contains("self")
    });
    ...
    using (var scope = app.Services.CreateScope())
    {
        ...
        await IdentitySeeder.SeedAsync(userManager, roleManager).ConfigureAwait(false);
        await DatabaseSeeder.SeedAsync(dbContext, userManager).ConfigureAwait(false);
    }
